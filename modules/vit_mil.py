...
class TransLayer(nn.Module):
    def __init__(self, norm_layer=nn.LayerNorm, dim=512,deterministic=True,dropout=0.,ffn=False,sdpa_type='torch',n_heads=8,no_norm=False,attn_type='sa',pos='',res=False):
        super().__init__()
...
class ViTMIL(nn.Module):
    def __init__(self, input_dim,n_classes,dropout,act,mil_norm=None,mil_bias=False,mil_cls_bias=True,inner_dim=512,pos=None,n_layers=1,deterministic=True,attn_dropout=0.,embed_feat=True,feat_embed_type='norm',embed_feat_mlp_ratio=4,ffn=False,sdpa_type='torch',n_heads=8,vit_norm=True,embed_norm_pos=0,attn_type='sa',attn_res=False,**kwargs):
        ...
        self.layer1 = TransLayer(dim=inner_dim,deterministic=deterministic,dropout=attn_dropout,ffn=ffn,sdpa_type=sdpa_type,n_heads=n_heads,no_norm=not vit_norm,attn_type=attn_type,pos=pos,res=attn_res)
        if n_layers >= 2:
            self.layer2 = TransLayer(dim=inner_dim,deterministic=deterministic,dropout=attn_dropout,ffn=ffn,sdpa_type=sdpa_type,n_heads=n_heads,no_norm=not vit_norm,pos=pos,res=attn_res)
        else:
